...
-   [交易标的对应最小价差](#交易标的对应最小价差 "交易标的对应最小价差")
-   [开始写策略](#开始写策略 "开始写策略")
...
-   [on\_order\_response(可选) - 委托主推](#on_order_response "on_order_response(可选) - 委托主推")
-   [on\_trade\_response(可选) - 交易主推](#on_trade_response "on_trade_response(可选) - 交易主推")
...
-   [get\_trading\_day\_by\_date - 按日期获取指定交易日](#get_trading_day_by_date "get_trading_day_by_date -  按日期获取指定交易日")
...
-   [获取证券信息](#获取证券信息 "获取证券信息")
-   [get\_stock\_name - 获取证券名称](#get_stock_name "get_stock_name - 获取证券名称")
-   [get\_stock\_info - 获取证券基础信息](#get_stock_info "get_stock_info - 获取证券基础信息")
-   [get\_stock\_status - 获取证券状态信息](#get_stock_status "get_stock_status - 获取证券状态信息")
-   [get\_underlying\_code - 获取证券的关联代码](#get_underlying_code "get_underlying_code- 获取证券的关联代码")
-   [get\_stock\_exrights - 获取证券除权除息信息](#get_stock_exrights "get_stock_exrights - 获取证券除权除息信息")
-   [get\_stock\_blocks - 获取证券所属板块信息](#get_stock_blocks "get_stock_blocks - 获取证券所属板块信息")
...
-   [get\_position - 获取单只标的持仓信息](#get_position "get_position - 获取单只标的持仓信息")
-   [get\_positions - 获取多只标的持仓信息](#get_positions "get_positions - 获取多只标的持仓信息")
-   [get\_all\_positions - 获取全部持仓信息](#get_all_positions "get_all_positions - 获取全部持仓信息")
...
-   [get\_deliver - 获取历史交割单信息](#get_deliver "get_deliver - 获取历史交割单信息")
-   [get\_fundjour - 获取历史资金流水信息](#get_fundjour "get_fundjour - 获取历史资金流水信息")
...
-   [get\_margin\_asset - 信用资产查询](#get_margin_asset "get_margin_asset - 信用资产查询")
...
-   [buy\_open - 开多](#buy_open "buy_open - 多开")
-   [sell\_close - 多平](#sell_close "sell_close - 多平")
-   [sell\_open - 空开](#sell_open "sell_open - 空开")
-   [buy\_close - 空平](#buy_close "buy_close - 空平")
...
-   [get\_dominant\_contract - 获取主力合约代码](#get_dominant_contract "get_dominant_contract - 获取主力合约代码")
...
-   [option\_buy\_open - 权利仓开仓](#option_buy_open "option_buy_open - 权利仓开仓")
-   [option\_sell\_close - 权利仓平仓](#option_sell_close "option_sell_close - 权利仓平仓")
-   [option\_sell\_open - 义务仓开仓](#option_sell_open "option_sell_open - 义务仓开仓")
-   [option\_buy\_close - 义务仓平仓](#option_buy_close "option_buy_close - 义务仓平仓")
...
-   [港股通专用函数](#港股通专用函数 "港股通专用函数")
-   [港股通查询类函数](#港股通查询类函数 "港股通查询类函数")
-   [get\_hks\_list - 获取港股通代码](#get_hks_list "get_hks_list - 获取港股通代码")
-   [get\_hks\_price\_gap - 港股通价差查询](#get_hks_price_gap "get_hks_price_gap - 港股通价差查询")
-   [get\_hks\_unit\_amount - 获取港股通标的委托单位数量](#get_hks_unit_amount "get_hks_unit_amount - 获取港股通标的委托单位数量")
-   [港股通交易类函数](#港股通交易类函数 "港股通交易类函数")
-   [hks\_order - 港股通买卖](#hks_order "hks_order - 港股通买卖")
-   [hks\_odd\_lot\_order - 港股通零股卖出](#hks_odd_lot_order "hks_odd_lot_order - 港股通零股卖出")
-   [计算函数](#计算函数 "计算函数")
...
-   [BarData - K线数据对象](#BarData "BarData - K线数据对象")
-   [Portfolio - 资产对象](#Portfolio "Portfolio - 资产对象")
-   [Position - 持仓对象](#Position "Position - 持仓对象")
-   [Order - 委托对象](#Order "Order - 委托对象")
-   [数据字典](#数据字典 "数据字典")
...
9:31(回测)/9:30(交易)~15:00分钟为盘中运行时间，分钟级别回测环境和交易环境支持运行在[run\_daily](#run_daily)中指定交易时间(如time='14:30')运行的函数；回测环境和交易环境支持运行[handle\_data](#handle_data)函数；交易环境支持运行[run\_interval](#run_interval)函数和[tick\_data](#tick_data)函数
...
[create\_dir(研究/回测/交易)](#create_dir)

[get\_frequency(回测/交易)](#get_frequency)

[get\_business\_type(回测/交易)](#get_business_type)

[set\_email\_info(交易)](#set_email_info)
...
[get\_trading\_day\_by\_date(研究/回测/交易)](#get_trading_day_by_date)

[get\_market\_list(研究/回测/交易)](#get_market_list)

[get\_market\_detail(研究/回测/交易)](#get_market_detail)

[get\_history(研究/回测/交易)](#get_history)
...
[get\_underlying\_code(交易)](#get_underlying_code)
...
[get\_trend\_data(研究/回测/交易)](#get_trend_data)
...
[get\_all\_positions(交易)](#get_all_positions)
...
[get\_lucky\_info(交易)](#get_lucky_info)
...
[margin\_trade(回测/交易)](#margin_trade)
...
[get\_margin\_asset(交易)](#get_margin_asset)
...
[get\_dominant\_contract(研究/回测/交易(期货))](#get_dominant_contract)
...
[option\_buy\_open(回测/交易(期权))](#option_buy_open)

[option\_sell\_close(回测/交易(期权))](#option_sell_close)

[option\_sell\_open(回测/交易(期权))](#option_sell_open)

[option\_buy\_close(回测/交易(期权))](#option_buy_close)
...
[get\_hks\_list(交易(港股通))](#get_hks_list)

[get\_hks\_price\_gap(交易(港股通))](#get_hks_price_gap)

[get\_hks\_unit\_amount(交易(港股通))](#get_hks_unit_amount)

[hks\_order(交易(港股通))](#hks_order)

[hks\_odd\_lot\_order(交易(港股通))](#hks_odd_lot_order)
...
[check\_limit(回测/交易)](#check_limit)
...
[filter\_stock\_by\_status(研究/回测/交易)](#filter_stock_by_status)

[get\_cb\_list(交易)](#get_cb_list)

[get\_reits\_list(研究/回测/交易)](#get_reits_list)

[get\_crdt\_fund(交易)](#get_crdt_fund)

[fund\_transfer(交易)](#fund_transfer)

[market\_fund\_transfer(交易)](#market_fund_transfer)
...
[get\_current\_kline\_count(研究/回测/交易)](#get_current_kline_count)
...
5.  当券商配置接收策略外交易产生的主推且策略调用[set\_parameters()](#set_parameters)并设置receive\_other\_response="1"时， 策略中将接收非本交易产生的主推。
6.  当策略调用[set\_parameters()](#set_parameters)并设置receive\_cancel\_response="1"， 策略接收到撤单成交主推时，主推信息中的order\_id为买入或卖出委托Order对象的order\_id，entrust\_no为撤单委托的委托编号。
...
order\_list：一个列表，当前委托单发生变化时，发生变化的委托单列表。委托单以字典形式展现，内容包括：'entrust\_no'(委托编号), 'error\_info'(错误信息), 'order\_time'(委托时间), 'stock\_code'(股票代码), 'amount'(委托数量), 'price'(委托价格), 'business\_amount'(成交数量), 'status'([委托状态](#委托状态)), 'entrust\_type'([委托类别](#委托类别)), 'entrust\_prop'([委托属性](#委托属性)), 'order\_id'([Order对象](#Order)编号)；
...
trade\_list：一个列表，当前成交单发生变化时，发生变化的成交单列表。成交单以字典形式展现，内容包括：'entrust\_no'(委托编号)，'business\_time'(成交时间)，'stock\_code'(股票代码)，'entrust\_bs'([委托方向](#委托方向))，'business\_amount'(成交数量)，'business\_price'(成交价格)，'business\_balance'(成交额)，'business\_id'(成交编号)，'status',([委托状态](#委托状态))(对接jz\_ufx、ctp期货柜台该字段为空)，'order\_id'([Order对象](#Order)编号)，'cancel\_info'(废单原因)，'withdraw\_no'(撤单原委托号)，'real\_type' ([成交类型](#成交类型)编号)，'real\_status'([成交状态](#成交状态)编号)；
...
-   trade\_status: [交易状态](#交易状态)(str:str)；
...
-   underlying\_type: [关联类型](#关联类型)(int)；
...
-   cash\_replace\_flag -- [现金替代标志](#现金替代标志)(str:str)；
...
market\_type：[市价委托类型](#市价委托类型)(int)，上证股票支持参数0、1、2、4，深证股票支持参数0、2、3、4、5，必传参数；
...
submarket\_type：[申购代码所属市场](#申购代码所属市场)，不传时默认申购全部新股(int)；
...
1.  该函数仅可撤[get\_all\_orders](#get_all_orders)函数返回的可撤状态订单。
...
-   amount： 委托数量(int)
-   entrust\_bs： [委托方向](#委托方向)(int)；
-   price： 委托价格(float)
-   status： [委托状态](#委托状态)(str)；
...
market\_type：[市价委托类型](#市价委托类型)(int)，上证股票支持参数0、1、2、4，深证股票支持参数0、2、3、4、5；
...
-   compact\_type:[合约类别](#合约类别)(str:str)；
-   compact\_source:合约来源(str:str)；
-   compact\_status:[合约状态](#合约状态)(str:str)；
...
-   exchange\_type:[交易类别](#交易类别)，仅包含1和2(str:str)；
-   entrust\_no:委托编号(str:str)；
-   compact\_type:[合约类别](#合约类别)(str:str)；
-   stock\_code:证券代码(str:str)；
...
以下属性也能通过[get\_history()](#get_history)/[get\_price()](#get_price)获取到
...
[Python3.5支持的三方库](/data/pip_list)

[Python3.11支持的三方库](/data/pip_list_py311)
...
def concat(objs, axis=0, join='outer', join\_axes=None, ignore\_index=False, keys=None, levels=None, names=None, verify\_integrity=False, sort=None, copy=True):

def concat(objs:Iterable\[NDFrame\]|Mapping\[HashableT, NDFrame\], axis:Axis=0, join:str="outer", ignore\_index:bool=False, keys=None, levels=None, names=None, verify\_integrity:bool=False, sort:bool=False, copy:bool=True)->DataFrame|Series:

弃用join\_axes参数

pandas.DatetimeIndex

def \_\_new\_\_(cls, data=None, freq=None, start=None, end=None, periods=None, tz=None, normalize=False, closed=None, ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None, copy=False, name=None, verify\_integrity=True):

def \_\_new\_\_(cls, data=None, freq:str|BaseOffset|lib.NoDefault=lib.no\_default, tz=None, normalize:bool=False, closed=None, ambiguous="raise", dayfirst:bool=False, yearfirst:bool=False, dtype:Dtype|None=None, copy:bool=False, name:Hashable=None)->DatetimeIndex:

弃用start参数

pandas.DataFrame.append

def append(self, other, ignore\_index=False, verify\_integrity=False, sort=None):

def append(self, other, ignore\_index:bool=False, verify\_integrity:bool=False, sort:bool=False)->DataFrame:

参数默认值改变

pandas.DataFrame.apply

def apply(self, func, axis=0, broadcast=None, raw=False, reduce=None, result\_type=None, args=(), \*\*kwds):

def apply(self, func:AggFuncType, axis:Axis=0, raw:bool=False, result\_type:Literal\["expand", "reduce", "broadcast"\]|None=None, args=(), \*\*kwargs):
...
def astype(self:NDFrameT, dtype, copy:bool\_t=True, errors:IgnoreRaise="raise")->NDFrameT:

不再支持以kwargs方式传入额外入参

pandas.DataFrame.quantile

def quantile(self, q=0.5, axis=0, numeric\_only=True, interpolation='linear'):

def quantile(self, q:float|AnyArrayLike|Sequence\[float\]=0.5, axis:Axis=0, numeric\_only:bool|lib.NoDefault=no\_default, interpolation:QuantileInterpolation="linear", method:Literal\["single", "table"\]="single")->Series|DataFrame:

参数默认值改变

pandas.DataFrame.replace

def replace(self, to\_replace=None, value=None, inplace=False, limit=None, regex=False, method='pad'):

def replace(self, to\_replace=None, value=lib.no\_default, inplace:bool=False, limit:int|None=None, regex:bool=False, method:Literal\["pad", "ffill", "bfill"\]|lib.NoDefault=lib.no\_default)->DataFrame|None:

参数默认值改变

pandas.DataFrame.resample

def resample(self, rule, how=None, axis=0, fill\_method=None, closed=None, label=None, convention='start', kind=None, loffset=None, limit=None, base=0, on=None, level=None):

def resample(self, rule, axis:Axis=0, closed:str|None=None, label:str|None=None, convention:str="start", kind:str|None=None, loffset=None, base:int|None=None, on:Level=None, level:Level=None, origin:str|TimestampConvertibleTypes="start\_day", offset:TimedeltaConvertibleTypes|None=None, group\_keys:bool|lib.NoDefault=no\_default, )->Resampler:

弃用how参数

pandas.DataFrame.sort\_index

def sort\_index(self, axis=0, level=None, ascending=True, inplace=False, kind='quicksort', na\_position='last', sort\_remaining=True, by=None):

def sort\_index(self, axis:Axis=0, level:IndexLabel=None, ascending:bool|Sequence\[bool\]=True, inplace:bool=False, kind:SortKind="quicksort", na\_position:NaPosition="last", sort\_remaining:bool=True, ignore\_index:bool=False, key:IndexKeyFunc=None)->DataFrame|None:

弃用by参数

pandas.DataFrame.to\_csv

def to\_csv(self, path\_or\_buf=None, sep=",", "", na\_rep='', float\_format=None, columns=None, header=True, index=True, index\_label=None, mode='w', encoding=None, compression=None, quoting=None, quotechar='"', line\_terminator='\\n', chunksize=None, tupleize\_cols=None, date\_format=None, doublequote=True, escapechar=None, decimal='.'):

def to\_csv(self, path\_or\_buf:FilePath|WriteBuffer\[bytes\]|WriteBuffer\[str\]|None=None, sep:str=",", na\_rep:str="", float\_format:str|Callable|None=None, columns:Sequence\[Hashable\]|None=None, header:bool\_t|list\[str\]=True, index:bool\_t=True, index\_label:IndexLabel|None=None, mode:str="w", encoding:str|None=None, compression:CompressionOptions="infer", quoting:int|None=None, quotechar:str='"', lineterminator:str|None=None, chunksize:int|None=None, date\_format:str|None=None, doublequote:bool\_t=True, escapechar:str|None=None, decimal:str=".", errors:str="strict", storage\_options:StorageOptions=None)->str|None:

参数默认值改变

pandas.DataFrame.to\_excel

def to\_excel(self, excel\_writer, sheet\_name='Sheet1', na\_rep='', float\_format=None, columns=None, header=True, index=True, index\_label=None, startrow=0, startcol=0, engine=None, merge\_cells=True, encoding=None, inf\_rep='inf', verbose=True, freeze\_panes=None):

def to\_excel(self, excel\_writer, sheet\_name:str="Sheet1", na\_rep:str="", float\_format:str|None=None, columns:Sequence\[Hashable\]|None=None, header:Sequence\[Hashable\]|bool\_t=True, index:bool\_t=True, index\_label:IndexLabel=None, startrow:int=0, startcol:int=0, engine:str|None=None, merge\_cells:bool\_t=True, encoding:lib.NoDefault=lib.no\_default, inf\_rep:str="inf", verbose:lib.NoDefault=lib.no\_default, freeze\_panes:tuple\[int, int\]|None=None, storage\_options:StorageOptions=None)->None:

参数默认值改变

pandas.DataFrame.to\_html

def to\_html(self, buf=None, columns=None, col\_space=None, header=True, index=True, na\_rep='NaN', formatters=None, float\_format=None, sparsify=None, index\_names=True, justify=None, bold\_rows=True, classes=None, escape=True, max\_rows=None, max\_cols=None, show\_dimensions=False, notebook=False, decimal='.', border=None, table\_id=None):

def to\_html(self, buf:FilePath|WriteBuffer\[str\]|None=None, columns:Sequence\[Level\]|None=None, col\_space:ColspaceArgType|None=None, header:bool|Sequence\[str\]=True, index:bool=True, na\_rep:str="NaN", formatters:FormattersType|None=None, float\_format:FloatFormatType|None=None, sparsify:bool|None=None, index\_names:bool=True, justify:str|None=None, max\_rows:int|None=None, max\_cols:int|None=None, show\_dimensions:bool|str=False, decimal:str=".", bold\_rows:bool=True, classes:str|list|tuple|None=None, escape:bool=True, notebook:bool=False, border:int|bool|None=None, table\_id:str|None=None, render\_links:bool=False, encoding:str|None=None)->str|None:

参数顺序改变

pandas.DataFrame.to\_json

def to\_json(self, path\_or\_buf=None, orient=None, date\_format=None, double\_precision=10, force\_ascii=True, date\_unit='ms', default\_handler=None, lines=False, compression=None, index=True):

def to\_json(self, path\_or\_buf:FilePath|WriteBuffer\[bytes\]|WriteBuffer\[str\]|None=None, orient:str|None=None, date\_format:str|None=None, double\_precision:int=10, force\_ascii:bool\_t=True, date\_unit:str="ms", default\_handler:Callable\[\[Any\], JSONSerializable\]|None=None, lines:bool\_t=False, compression:CompressionOptions="infer", index:bool\_t=True, indent:int|None=None, storage\_options:StorageOptions=None)->str|None:

参数默认值改变

pandas.DataFrame.to\_records

def to\_records(self, index=True, convert\_datetime64=None):

def to\_records(self, index:bool=True, column\_dtypes=None, index\_dtypes=None)->np.recarray:

弃用convert\_datetime64参数

pandas.DataFrame.to\_string

def to\_string(self, buf=None, columns=None, col\_space=None, header=True, index=True, na\_rep='NaN', formatters=None, float\_format=None, sparsify=None, index\_names=True, justify=None, line\_width=None, max\_rows=None, max\_cols=None, show\_dimensions=False):

def to\_string(self, buf:FilePath|WriteBuffer\[str\]|None=None, columns:Sequence\[str\]|None=None, col\_space:int|list\[int\]|dict\[Hashable, int\]|None=None, header:bool|Sequence\[str\]=True, index:bool=True, na\_rep:str="NaN", formatters:fmt.FormattersType|None=None, float\_format:fmt.FloatFormatType|None=None, sparsify:bool|None=None, index\_names:bool=True, justify:str|None=None, max\_rows:int|None=None, max\_cols:int|None=None, show\_dimensions:bool=False, decimal:str=".", line\_width:int|None=None, min\_rows:int|None=None, max\_colwidth:int|None=None, encoding:str|None=None)->str|None:

参数顺序改变

pandas.DataFrame.update

def update(self, other, join='left', overwrite=True, filter\_func=None, raise\_conflict=False):

def update(self, other, join:str="left", overwrite:bool=True, filter\_func=None, errors:str="ignore")->None:

弃用raise\_conflict参数

pandas.Panel

def \_\_init\_\_(self, data=None, items=None, major\_axis=None, minor\_axis=None, copy=False, dtype=None):

在pandas(0.25.0)版本已弃用

pandas.read\_excel

def read\_excel(io, sheet\_name=0, header=0, names=None, index\_col=None, usecols=None, squeeze=False, dtype=None, engine=None, converters=None, true\_values=None, false\_values=None, skiprows=None, nrows=None, na\_values=None, parse\_dates=False, date\_parser=None, thousands=None, comment=None, skipfooter=0, convert\_float=True, \*\*kwds):

def read\_excel(io, sheet\_name:str|int|list\[IntStrT\]|None=0, header:int|Sequence\[int\]|None=0, names:list\[str\]|None=None, index\_col:int|Sequence\[int\]|None=None, usecols:int|str|Sequence\[int\]|Sequence\[str\]|Callable\[\[str\], bool\]|None=None, squeeze:bool|None=None, dtype:DtypeArg|None=None, engine:Literal\["xlrd", "openpyxl", "odf", "pyxlsb"\]|None=None, converters:dict\[str, Callable\]|dict\[int, Callable\]|None=None, true\_values:Iterable\[Hashable\]|None=None, false\_values:Iterable\[Hashable\]|None=None, skiprows:Sequence\[int\]|int|Callable\[\[int\], object\]|None=None, nrows:int|None=None, na\_values=None, keep\_def ault\_na:bool=True, na\_filter:bool=True, verbose:bool=False, parse\_dates:list|dict|bool=False, date\_parser:Callable|None=None, thousands:str|None=None, decimal:str=".", comment:str|None=None, skipfooter:int=0, convert\_float:bool|None=None, mangle\_dupe\_cols:bool=True, storage\_options:StorageOptions=None)->DataFrame|dict\[IntStrT, DataFrame\]:

参数默认值改变

pandas.read\_json

def read\_json(path\_or\_buf=None, orient=None, typ='frame', dtype=True, convert\_axes=True, convert\_dates=True, keep\_def ault\_dates=True, numpy=False, precise\_float=False, date\_unit=None, encoding=None, lines=False, chunksize=None, compression='infer'):

def read\_json(path\_or\_buf:FilePath|ReadBuffer\[str\]|ReadBuffer\[bytes\], orient:str|None=None, typ:Literal\["frame", "series"\]="frame", dtype:DtypeArg|None=None, convert\_axes=None, convert\_dates:bool|list\[str\]=True, keep\_def ault\_dates:bool=True, numpy:bool=False, precise\_float:bool=False, date\_unit:str|None=None, encoding:str|None=None, encoding\_errors:str|None="strict", lines:bool=False, chunksize:int|None=None, compression:CompressionOptions="infer", nrows:int|None=None, storage\_options:StorageOptions=None)->DataFrame|Series|JsonReader:

参数默认值改变

pandas.testing.assert\_frame\_equal

def assert\_frame\_equal(left, right, check\_dtype=True, check\_index\_type='equiv', check\_column\_type='equiv', check\_frame\_type=True, check\_less\_precise=False, check\_names=True, by\_blocks=False, check\_exact=False, check\_datetimelike\_compat=False, check\_categorical=True, check\_like=False, obj='DataFrame'):

def assert\_frame\_equal(left, right, check\_dtype:bool|Literal\["equiv"\]=True, check\_index\_type:bool|Literal\["equiv"\]="equiv", check\_column\_type="equiv", check\_frame\_type=True, check\_less\_precise=no\_default, check\_names=True, by\_blocks=False, check\_exact=False, check\_datetimelike\_compat=False, check\_categorical=True, check\_like=False, check\_freq=True, check\_flags=True, rtol=1.0e-5, atol=1.0e-8, obj="DataFrame")->None:

参数默认值改变

pandas.to\_datetime

def to\_datetime(arg, errors='raise', dayfirst=False, yearfirst=False, utc=None, box=True, format=None, exact=True, unit=None, infer\_datetime\_format=False, origin='unix', cache=False):

def to\_datetime(arg:DatetimeScalarOrArrayConvertible|DictConvertible, errors:DateTimeErrorChoices="raise", dayfirst:bool=False, yearfirst:bool=False, utc:bool|None=None, format:str|None=None, exact:bool=True, unit:str|None=None, infer\_datetime\_format:bool=False, origin="unix", cache:bool=True)->DatetimeIndex|Series|DatetimeScalar|NaTType|None:

弃用box参数

pandas.to\_pickle

def to\_pickle(obj, path, compression='infer', protocol=pkl.HIGHEST\_PROTOCOL):

def to\_pickle(obj:Any, filepath\_or\_buffer:FilePath|WriteBuffer\[bytes\], compression:CompressionOptions="infer", protocol:int=pickle.HIGHEST\_PROTOCOL, storage\_options:StorageOptions=None) -> None:

protocol表示二进制数据序列化协议，Python3.5默认使用4版本协议，Python3.11默认使用5版本协议。使用5版本协议保存的pickle文件将无法在Python3.5版本读取。
...
1.  [run\_interval()](#run_interval)新增支持interval\_timer\_ranges(设置指定函数运行的时间范围)入参；

PTrade1.0-QTV202401.05.031

1.  Python3.5新增三方库：gmssl==3.2.2，psycopg2-binary==2.8.6
2.  Python3.11新增三方库：gmssl==3.2.2，psycopg2-binary==2.9.11

PTrade1.0-QTV202401.05.030

1.  新增[get\_fundamentals](#get_fundamentals)balance\_statement表返回字段增加contract\_liability、total\_fixed\_asset、t\_constru\_in\_process字段，income\_statement表返回字段增加r\_and\_d字段；

PTrade1.0-QTV202401.05.021

1.  [get\_market\_list()](#get_market_list)仅返回系统支持的四个市场；
2.  [get\_market\_detail()](#get_market_detail)仅支持入参系统支持的四个市场；
3.  Python3.11新增三方库：dtaidistance==2.3.13

PTrade1.0-QTV202401.05.013

1.  [get\_price()](#get_price)接口中的fq字段新增支持dypre-动态前复权；
2.  [get\_price()](#get_price)接口异常返回空字典改为返回值为None；

PTrade1.0-QTV202401.05.011

1.  Python3.11升级三方库：sxsc-tushare==1.2.11-->sxsc-tushare==20240927
2.  Python3.5升级三方库：sxsc-tushare==1.2.11-->sxsc-tushare==20240927

PTrade1.0-QTV202401.05.008

1.  [Portfolio](#Portfolio)期货返回字段删除capital\_used，增加margin；

PTrade1.0-QTV202401.05.000

1.  Python3.11升级三方库：bs4==0.0.1-->bs4==0.0.2
2.  Python3.11升级三方库：gensim==4.3.0-->gensim==4.3.3
3.  Python3.11升级三方库：jax==0.4.13-->jax==0.4.18
4.  Python3.11升级三方库：jaxlib==0.4.13-->jaxlib==0.4.18
5.  Python3.11升级三方库：line\_profiler==4.0.2-->line\_profiler==4.1.3
6.  Python3.11升级三方库：qdldl==0.1.7.post0-->qdldl==0.1.7.post4
7.  Python3.11升级三方库：TA-Lib==0.4.25-->TA-Lib==0.4.31
8.  Python3.11新增三方库：asyncache==0.3.1, fastapi==0.100.0, h11==0.14.0, pydantic==1.10.7, starlette==0.27.0, uvicorn==0.30.6, baostock==0.8.9
9.  Python3.5新增三方库：baostock==0.8.9
10.  股票[Position](#Position)对象展示字段新增update\_time(持仓更新时间)；
11.  新增 [成交类型](#成交类型)字典；
12.  新增 [成交状态](#成交状态)字典；

PTrade1.0-QTV202401.04.000

1.  新增[fund\_transfer()](#fund_transfer)资金调拨；
2.  新增[market\_fund\_transfer()](#market_fund_transfer)市场间资金调拨；
3.  新增[set\_email\_info()](#set_email_info)设置邮件信息；
4.  [on\_trade\_response()](#on_trade_response)新增返回real\_type: 成交类型、real\_status: 成交状态两个字段；
5.  [run\_interval()](#run_interval)seconds最小运行间隔时间的设置规则修改为期货最小1秒，股票等其他业务最小3秒；
6.  [create\_dir()](#create_dir)出参None修改为是否创建成功(True/False)；
7.  Python3.5新增三方库：walrus==0.9.3

PTrade1.0-QTV202401.02.000

1.  [margincash\_open()](#margincash_open)字段cash\_group不入参修改为默认表示普通头寸；
2.  [margincash\_close()](#margincash_close)增加cash\_group字段入参；
3.  [margincash\_direct\_refund()](#margincash_direct_refund)增加cash\_group字段入参；
4.  [marginsec\_open()](#marginsec_open)字段cash\_group不入参修改为默认表示普通头寸；
5.  [marginsec\_direct\_refund()](#marginsec_direct_refund)增加cash\_group字段入参；
6.  [get\_margincash\_open\_amount()](#get_margincash_open_amount)字段cash\_group不入参修改为默认表示普通头寸；
7.  [get\_margincash\_close\_amount()](#get_margincash_close_amount)增加cash\_group字段入参；
8.  [get\_marginsec\_open\_amount()](#get_marginsec_open_amount)字段cash\_group不入参修改为默认表示普通头寸；
9.  [get\_marginsec\_close\_amount()](#get_marginsec_close_amount)增加cash\_group字段入参；
10.  [get\_margin\_entrans\_amount()](#get_margin_entrans_amount)增加cash\_group字段入参；
11.  [get\_enslo\_security\_info()](#get_enslo_security_info)增加cash\_group字段入参；
12.  [get\_crdt\_fund()](#get_crdt_fund)增加get\_crdt\_fund接口；
13.  [get\_margin\_contract()](#get_margin_contract)新增compact\_source合约来源字段入参和返回；
14.  Python3.5新增三方库：memory-profiler==0.61.0, psutil==5.9.5

PTrade1.0-QTV202401.01.000

1.  [set\_parameters()](#set_parameters)新增入参not\_restart\_trade、server\_restart\_not\_do\_before；
2.  [委托属性()](#委托属性)新增字段:"4"-回购；
...
3.  [get\_deliver()](#get_deliver)返回字段entrust\_no,report\_no类型由int改为str；
4.  [set\_benchmark()](#set_benchmark)入参字段security改为sids；
5.  [option\_buy\_open()](#option_buy_open)期权权利仓开仓接口由buy\_open调整为option\_buy\_open；
6.  [option\_buy\_close()](#option_buy_close)期权权利仓平仓接口由buy\_close调整为option\_buy\_close；
7.  [option\_sell\_open()](#option_sell_open)期权义务仓开仓接口由sell\_open调整为option\_sell\_open；
8.  [option\_sell\_close()](#option_sell_close)期权义务仓平仓接口由sell\_close调整为option\_sell\_close；
9.  新增[check\_strategy()](#check_strategy)检查策略内容；
10.  新增[get\_dominant\_contract()](#get_dominant_contract)获取期货主力合约；
11.  Python3.5新增三方库：flameprof==0.4, pypinyin==0.50.0
12.  Python3.11新增三方库：pypinyin==0.50.0

PTrade1.0-QTV202301.03.000

1.  弃用get\_individual\_transcation()；
2.  弃用get\_margin\_assert()；
3.  [check\_limit()](#check_limit)新增支持在研究和回测模块使用；
4.  [ipo\_stocks\_order()](#ipo_stocks_order)入参market\_type修改为submarket\_type；
5.  [get\_enslo\_security\_info()](#get_enslo_security_info)出参market\_type修改为exchange\_type；
6.  [get\_etf\_info()](#get_etf_info)返回参数pre\_cash\_componet修改为pre\_cash\_component；
7.  [get\_fundamentals()](#get_fundamentals)入参date(查询日期)新增支持datetime.date类型、新增支持is\_dataframe(是否返回DataFrame类型)入参、弃用date\_type入参；
8.  [get\_hks\_unit\_amount()](#get_hks_unit_amount)入参entrust\_type修改为trade\_type；
9.  [set\_parameters()](#set_parameters)不再支持individual\_data\_in\_dict、tick\_direction\_in\_dict入参；
10.  [get\_individual\_entrust()](#get_individual_entrust)、[get\_individual\_transaction()](#get_individual_transaction)和[get\_tick\_direction()](#get_tick_direction)新增支持is\_dict(是否返回字典类型数据)入参；
11.  [get\_margin\_contractreal()](#get_margin_contractreal)出参market\_type修改为exchange\_type；
12.  [get\_gear\_price()](#get_gear_price)不再支持在回测中调用；
13.  [get\_stock\_status()](#get_stock_status)入参query\_type(查询类型)新增支持查询DELISTING\_SORTING(是否退市整理期)类型；
14.  [get\_trades()](#get_trades)在期货回测场景下新增返回开平仓类型字段；
15.  新增[filter\_stock\_by\_status()](#filter_stock_by_status)过滤指定状态的股票代码；
16.  新增[get\_current\_kline\_count()](#get_current_kline_count)获取股票业务当前时间的分钟bar数量；
17.  新增[get\_trading\_day\_by\_date()](#get_trading_day_by_date)按日期获取指定交易日；
18.  股票[Position](#Position)对象展示字段新增today\_amount(今日开仓数量)；
19.  新增三方库：walrus==0.9.3, gqdata==0.1.5, mplfinance==0.12.10b0
20.  新增支持Python3.11版本，可通过终端右上角选择Python版本；
21.  由于Pandas库0.25.0版本后不再支持Panel类型，在Python3.11版本环境中[get\_fundamentals()](#get_fundamentals)按年份查询模式、[get\_history()](#get_history)、[get\_individual\_entrust()](#get_individual_entrust)、[get\_individual\_transaction()](#get_individual_transaction)和[get\_price()](#get_price)默认返回DataFrame类型；

PTrade1.0-QTV202301.02.000

1.  [get\_snapshot()](#get_snapshot)不再返回close\_px(今日收盘)、avg\_px(均价)字段；
2.  [get\_price()](#get_price)、[get\_history()](#get_history)获取日K线新增返回is\_open字段；
3.  删除三方库：arch==3.2, cvxopt==1.1.8
4.  新增三方库：PuLP==2.7.0
5.  研究新增支持[get\_history()](#get_history)获取历史行情；
6.  新增[get\_business\_type()](#get_business_type)获取当前策略的业务类型；
7.  新增[get\_ipo\_stocks()](#get_ipo_stocks)获取当日IPO申购标的；

PTrade1.0-QTV202301.01.000

1.  委托流程调整，交易模式中非交易时间段内调用两融、盘后固定价等API产生的委托直接发送柜台进行处理；
2.  [Order](#Order)对象展示字段新增cancel\_entrust\_no(撤单委托编号)；
3.  期货[Position](#Position)对象去除margin\_rate(保证金比例)字段，增加margin(持仓保证金)字段；
4.  [run\_interval()](#run_interval)入参seconds(最小执行周期)由最小3s修改为最小0.1s限制；
5.  [get\_trades()](#get_trades)返回数据中的委托编号字段数据类型统一由int修改为str，成交时间字段格式统一为YYYY-mm-dd HH:MM:SS；
6.  [order\_market()](#order_market)新增返回order\_id(Order订单编号)字段；
7.  [margincash\_close()](#margincash_close)、[marginsec\_close()](#marginsec_close)新增支持market\_type(市价委托类型)入参；
8.  [get\_price()](#get_price)、[get\_history()](#get_history)新增支持is\_dict(是否返回字典类型数据入参)；
9.  新增[get\_margin\_asset()](#get_margin_asset)信用资产查询，后续版本将弃用get\_margin\_assert()；
10.  新增[get\_individual\_transaction()](#get_individual_transaction)获取逐笔成交行情，后续版本将弃用get\_individual\_transcation()；
11.  新增[get\_reits\_list()](#get_reits_list)获取基础设施公募REITs基金代码列表；

PTrade1.0-QTV202301.00.000

1.  [get\_margincash\_open\_amount()](#get_margincash_open_amount)新增支持cash\_group(两融头寸性质)入参；
2.  [get\_all\_orders()](#get_all_orders)新增返回entrust\_time(委托时间)字段；
3.  [get\_open\_orders()](#get_open_orders)改成引擎每天收盘后清空对应的未完成订单列表；
4.  新增[get\_cb\_info()](#get_cb_info)获取可转债基础信息；
5.  新增[get\_enslo\_security\_info()](#get_enslo_security_info)融券信息查询；
6.  新增[get\_trend\_data()](#get_trend_data)获取集中竞价期间的代码数据；

PBOXQT1.0V202202.03.000

1.  [get\_user\_name()](#get_user_name)新增支持login\_account(返回当前交易类型对应账号)入参；
2.  [debt\_to\_stock\_order()](#debt_to_stock_order)债转股委托新增支持在两融交易中调用；
3.  [get\_instruments()](#get_instruments)新增返回changepct\_limit (每日涨跌幅度)、littlest\_changeunit (最小变动价位)字段；
4.  [order\_market()](#order_market)和[margin\_trade()](#margin_trade)做市价委托上证股票时，limit\_price(保护限价)为必传字段；
5.  [send\_email()](#send_email)单个交易中每分钟调用次数做限制，一分钟最多发送一次；
6.  新增三方库：pykalman==0.9.5
7.  新增[get\_all\_positions()](#get_all_positions)获取全部持仓；
8.  新增[get\_lucky\_info()](#get_lucky_info)获取历史中签信息；
9.  新增[get\_future\_main\_code()](#get_future_main_code)获取主力合约代码；

PBOXQT1.0V202202.02.000

1.  [get\_price()](#get_price)、[get\_history()](#get_history)新增支持获取期货数据；
2.  [get\_snapshot()](#get_snapshot)新增返回iopv(基金份额参考净值)字段；
3.  [handle\_data(context, data)](#handle_data)中data对象帮助描述由SecurityUnitData修改为BarData，BarData对象新增返回dt(当前周期时间)、preclose(昨收盘价(仅日线返回))、high\_limit(涨停价(仅日线返回))、low\_limit(跌停价(仅日线返回))、unlimited(是否无涨跌停限制(仅日线返回))字段，字段描述详见接口说明；BarData对象中dt(当前周期时间)字段值由UTC时间修改为北京时间；
4.  [on\_trade\_response()](#on_trade_response)新增返回withdraw\_no(撤单原委托号)、cancel\_info(废单原因)字段；返回字段值中entrust\_no(委托编号)、withdraw\_no(撤单原委托号)统一转为str类型；
5.  [set\_parameters()](#set_parameters)新增支持individual\_data\_in\_dict([get\_individual\_entrust()](#get_individual_entrust)、[get\_individual\_transaction()](#get_individual_transaction)获取逐笔数据API返回字典类型)、tick\_direction\_in\_dict([get\_tick\_direction()](#get_tick_direction)获取分时成交数据API返回字典类型)参数；
6.  [set\_yesterday\_position()](#set_yesterday_position)新增支持设置ETF、LOF类型的底仓；
7.  [margincash\_open()](#margincash_open)、[marginsec\_open()](#marginsec_open)新增cash\_group(两融头寸性质)入参；
8.  [order\_market()](#order_market)和[margin\_trade()](#margin_trade)做市价委托上证股票时，limit\_price(保护限价)为必传字段。
9.  [get\_index\_stocks()](#get_index_stocks)由支持多个指数查询修改为仅支持单个指数查询
10.  由于Keras库2.3.1版本与TensorFlow库不兼容，需要降版本：Keras==2.3.1-->Keras==2.2.4。
11.  新增[get\_frequency()](#get_frequency)获取当前业务代码的周期；
12.  新增[get\_underlying\_code()](#get_underlying_code) 获取代码关联的代码；
13.  新增[get\_hks\_list()](#get_hks_list)获取港股通代码；
14.  新增[get\_hks\_price\_gap()](#get_hks_price_gap)港股通价差查询；
15.  新增[get\_hks\_unit\_amount()](#get_hks_unit_amount)获取港股通标的委托单位数量；
16.  新增[hks\_order()](#hks_order)港股通买卖；
17.  新增[hks\_odd\_lot\_order()](#hks_odd_lot_order)港股通零股卖出；
...
7.  交易场景中[buy\_open()](#buy_open)、[sell\_close()](#sell_close)、[sell\_open()](#sell_open)、[buy\_close()](#buy_close)新增支持期权业务；
8.  [on\_trade\_response()](#on_trade_response)新增接收撤单委托的成交主推，详见接口说明注意事项；
...
19.  [get\_snapshot()](#get_snapshot)新增返回avg\_px(均价)、business\_count(成交笔数)、close\_px(今收价)、end\_trade\_date(最后交易日、settlement(结算价)、start\_trade\_date(首个交易日)、tick\_size(最小报价单位)、trade\_mins(交易分钟数)字段，去除prod\_code(证券代码)字段说明；
20.  新增[open\_prepared()](#open_prepared)备兑开仓；
21.  新增[close\_prepared()](#close_prepared)备兑平仓；
22.  新增[option\_exercise()](#option_exercise)行权；
23.  新增[set\_parameters()](#set_parameters)设置策略配置参数，支持的参数详见接口说明；
...
10.  [run\_daily()](#run_daily)、[run\_interval()](#run_interval)可以在initialize中多次调用，累计不可超过5次，详见接口说明注意事项；
...
1.  [get\_snapshot](#get_snapshot)新增issue\_date字段说明；
...
6.  [set\_commission()](#set_commission)增加type=“LOF”类型基金费率设置的功能；
7.  新增支持期货日盘回测功能、期货日盘交易功能(对接UFT柜台)，期货API接口详见量化帮助文档[期货专用函数](#期货专用函数)模块；
8.  新增[get\_tick\_direction()](#get_tick_direction)获取分时成交行情；
9.  新增[get\_sort\_msg()](#get_sort_msg)获取版块、行业的涨幅排名；
10.  新增[permission\_test()](#permission_test)权限校验；
...
2.  [get\_market\_list()](#get_market_list)限制仅[before\_trading\_start](#before_trading_start)和[after\_trading\_end](#after_trading_end)中使用；
3.  [get\_snapshot()](#get_snapshot)新增返回hsTimeStamp(快照时间戳)字段；对接L2行情买卖一档新增返回委托队列；
4.  [ipo\_stocks\_order()](#ipo_stocks_order)新增black\_stocks(新股/债黑名单)入参；
5.  [on\_order\_response()](#on_order_response)新增返回error\_info(错误信息)字段；
...
4.  修复交易场景中获取当日K线14:58、14:59分价格为0的问题；
