# 克隆自聚宽文章：https://www.joinquant.com/post/71084
# 标题：七星高照5.1+五福闹春5.0+三马七星1.7实测收益对比
# 作者：蓝色沸点38
...
def get_small_cap_stocks_v3(context):
    ...
    q = (
        ...
        .filter(
            valuation.code.in_(initial_list),
            valuation.market_cap.between(50, 350),
            income.operating_revenue > 1e8,
            indicator.roe > 0,
            indicator.roa > 0,
...
def is_security_pause(context,code):
    timeStr = context.current_dt.strftime("%Y-%m-%d %H:%M:00")
    prices = get_bars(code, count=5, unit='1m', fields=['date', 'close','volume'],
include_now=False, end_dt=timeStr, fq_ref_date=None)
    volumeSum = 0
    if len(prices): 
...
def get_ranked_etfs_172(context):
    ...
    for etf in g.etf_pool_3:
        # 不能用paused，上午开盘下午停盘，聚宽依然返回开盘
        # if current_data[etf].paused:
        #     continue
        # 使用最近5分钟成交量为0替代停牌判断
        if is_security_pause(context,etf):
            continue
...
def smart_order_target_value_172(security, target_value, context):
    data = get_current_data()
    if is_security_pause(context,security):
        return False
...
def morning_greet(context):
    log.info(f"🌞 策略存活确认，当前日期: {context.current_dt.strftime('%Y-%m-%d %H:%M:%S')}")
...
def schedule_tasks(context):
    ...
    if g.portfolio_value_proportion[2] > 0:
        ...
        run_daily(strategy_3_buy, time='14:02')
