...
#行业ETF：
    ...
    run_daily(check_and_sell, time='13:07')
    run_daily(check_and_buy, time='13:09')
    run_daily(check_sim_real_position, time='15:08')
...
def check_and_sell(context):
    log.info("▶️ 【卖出流水线】启动，只卖不买")
...
def check_sim_real_position(context):
    """
...
def smart_order_target_value(security, target_value, context):
    ...
    if order_result:
        if amount_diff > 0:
            log.info(f"📦 买入 {security} {security_name} 数量:{amount_diff}，当前价：{current_price}")
        else:
            log.info(f"📤 卖出 {security} {security_name} 数量:{abs(amount_diff)} 当前价：{current_price}")
        return True
    return False
