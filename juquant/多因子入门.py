...
def set_feasible_stocks(stock_list, days, context):
    ...
    for stock in stock_list:
        ...
        # 2. 历史 days 天内是否曾停牌
        hist = attribute_history(stock, days, '1d', fields=['paused'], skip_paused=False)
        if hist['paused'].sum() == 0:
...
def handle_data(context, data):
    ...
    # 计算现在的总资产，以分配资金，这里是等额权重分配
        g.everyStock=context.portfolio.available_cash/g.N
        # 获得今天日期的字符串
        todayStr=str(context.current_dt)[0:10]
        yesterday=str(context.previous_date)[0:10]
        # 获得因子排序
        a,b=getRankedFactors(g.factors,yesterday)
        # 计算每个股票的得分
...
def order_stock_sell(context, data, toBuy):
    ...
    for stock in list(context.portfolio.positions.keys()):
        if stock not in toBuy:
            # 1. 停牌检查
            if current_data[stock].paused:
                log.warning("股票 %s 今日停牌，无法卖出，已保留", stock)
                continue
            
            # 2. 持仓数量检查
            current_amount = context.portfolio.positions[stock].total_amount
            log.warning("计划卖出股票 %s 持仓 %d 股", stock, current_amount)
...
def order_stock_buy(context, data, toBuy):
    ...
    for i in range(0, len(g.all_stocks)):
        if indexOf(g.all_stocks[i], toBuy) > -1:
            stock = g.all_stocks[i]
            # 额外检查停牌（理论上不需要，但安全起见）
            if current_data[stock].paused:
                log.warning("股票 %s 停牌，跳过买入", stock)
                continue
            
            target_val = g.everyStock
            safe_order_target_value(stock, target_val)

def safe_order_target_value(stock, target_value):
    current_price = get_current_data()[stock].last_price
    # 计算最少需要多少资金才能买 100 股
    min_required = 100 * current_price
    if target_value < min_required:
        log.warning("股票 %s 目标资金 %.2f 元不足以买入100股（现价%.2f），跳过", stock, target_value, current_price)
        return
    log.info("计划买入股票 %s，%s元，单价%s元", stock,target_value,current_price)
    if stock.startswith('688'):
        limit_price = current_price * 1.01
        order_target_value(stock, target_value, LimitOrderStyle(limit_price))
    else:
