...
def initialize(context):
    ...
    g.momentum_threshold_low = -0.015
    ...
    # ========== 优化6参数（单只ETF止损） ==========
    g.stop_loss_drawdown = 0.05
    g.stop_loss_half_ratio = 0.5
    ...
    # 每日止损检查（优化6）
    run_daily(stop_loss_check, time='14:50')
...
def stop_loss_check(context):
    ...
    for code, pos in positions.items():
        if pos.value <= 0 or code == g.cash_etf:
            continue
        ...
        if drawdown >= g.stop_loss_drawdown:
            ...
            log.info("优化6止损触发：%s 从高点 %.3f 回撤 %.2f%%，减仓至 %.0f元" % 
...
def apply_volatility_target(position_ratio, context, selected_with_weights):
    ...
    log.info(f"改进1波动率目标: 组合波动率{total_vol:.2%}, 调整系数{adj:.2f}, 仓位{position_ratio:.2f}->{new_ratio:.2f}")
    return new_ratio
