...
def set_targets(context):
    g.etf_only = [
        '159919.XSHE','159952.XSHE','510500.XSHG','512880.XSHG',
        '588090.XSHG','512100.XSHG','159533.XSHE','159996.XSHE',
        '159941.XSHE','159612.XSHE',
        '159934.XSHE','513010.XSHG','515220.XSHG','159981.XSHE','159985.XSHE',
        '159755.XSHE','561380.XSHG','159129.XSHE','515070.XSHG',
        '516770.XSHG','159565.XSHE','159770.XSHE','159938.XSHE','159928.XSHE',
    ]
    g.stock_only = list({
        # 原版
        '002837.XSHE',#英维克
...
def initialize(context):
    ...
    g.gold_etf = '159934.XSHE'
    # g.nasdaq_etf = '513100.XSHG'
    g.cash_etf = '511010.XSHG'
    g.sp500_etf = '513650.XSHG'
    g.safe_etfs = [g.gold_etf, g.cash_etf]
...
def schedule_tasks(context):
    ...
    # run_daily(open_stop_loss_check, "10:00")
    run_daily(compute_signals, '14:45')
    run_daily(execute_sell_stop, '14:45')
...
def order_by_pct(context, code, target_pct, total_value,side='buy'):
    ...
    if diff != 0:
        if 'buy' == side and diff > 0: 
            ...
            # 只操作卖出
            order_target_value(code, value)
    log.info(f"order_by_pct {code} 调仓{side} {diff} 目标股数={s} 目标仓位{target_pct} 金额{value} 原有股数={cv} 现价={px} ")
...
def check_market(context):
    ...
    if out_bear_1 or out_bear_2 or out_bear_3 or out_bear_4 or out_bear_5:
        g.bear_mode = False
        log.info(f"[大盘] 创业板3周MA上穿13周MA → 牛市")
    
    log.info(f"{g.benchmark_code} 现价={cur_price} 最终结果熊市={g.bear_mode} {p3:.3f} {p13:.3f} {c3:.3f} {c13:.3f} {above13_0}")
    return not g.bear_mode
...
def check_stop_single(context, code):
    ...
    if pos.total_amount==0: g.high_prices.pop(code,None); return
    closeable_amount = pos.closeable_amount
    if closeable_amount <= 0:
        return
    try: px=get_current_data()[code].last_price
    except: return
    ...
    if lp>g.stop_loss_pct:
        log.info(f"check_stop_single 跌幅止损卖出{code} 亏损={lp*100}% 阈值={g.stop_loss_pct*100}%")
    ...
    try:
        ...
        if len(df)>=g.atr_period:
            ...
            if px < mp :
                log.info(f"check_stop_single 价格跌破止损卖出{code} 现价={px} 阈值均价={mp}")
                order(code,-closeable_amount)
                g.high_prices.pop(code,None); g.entry_dates.pop(code,None); g.last_rebalance.pop(code,None)
    except: pass
...
def open_stop_loss_check(context):
    """每日检查持仓ETF是否触发回撤止损"""
    current_data = get_current_data()
    positions = context.portfolio.positions
    for code, pos in positions.items():
        if pos.closeable_amount <= 0 or code in g.safe_etfs:
            continue
        current_price = current_data[code].last_price
        g.high_prices[code]=max(g.high_prices.get(code,current_price),current_price)
        ...
        high = g.high_prices[code]
        drawdown = (high - current_price) / high
        lower_ma5 = current_price<(ma5*0.97)
        
        if (drawdown >= g.stop_loss_pct) or lower_ma5:
            log.info(f"开盘止损触发：{code} 从高点 {high:.3f} 回撤 {drawdown:.3%} 低于5日线{lower_ma5}")
...
def compute_signals(context):
    ...
    log.info(f"check_market bear={g.bear_mode}")
    bear = g.bear_mode
    if bear:
...
def rebalance(context,targets,pct,side='buy'):
    ...
    for code in targets:
        ...
        if ld and (today - ld).days < g.rebalance_freq_days:
            continue
        log.info(f"rebalance {code} {side} pct={pct}")
        ...
        try:
            ...
            log.error(f"rebalance error {code} {pos}")
            pass
