...
def wrap_orders(_order, _order_target_value, _order_value, _order_target,live=False):
    global _ORDERS_FUNC
    _ORDERS_FUNC['order'] = _order
    _ORDERS_FUNC['order_target_value'] = _order_target_value
    _ORDERS_FUNC['order_value'] = _order_value
    _ORDERS_FUNC['order_target'] = _order_target
    _ORDERS_FUNC['live'] = live
...
class RemoteBrokerClient:
    ...
    def _place_order(self, security: str, amount: int, price: Optional[float], side: str, wait_timeout: float) -> RemoteOrder:
        ...
        try:
            ...
            order_dict = self.get_orders(order_id=order.order_id)  # 刷新订单状态
            order = order_dict.get(order.order_id, order)  # 更新订单状态，如果查询失败则使用原订单对象
            return order
